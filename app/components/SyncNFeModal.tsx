...
interface SyncNFeModalProps {
  isOpen: boolean;
  onCancel: () => void;
  onSyncComplete?: () => void;
}

export default function SyncNFeModal({ isOpen, onCancel, onSyncComplete }: SyncNFeModalProps) {
  ...
  async function handleSync() {
    ...
    try {
      ...
      const response = await fetch("/api/clientes/sincronizar-nfe", {
        method: "POST",
        headers: {
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/78 px-4 py-6 backdrop-blur-sm">
      <section className="w-full max-w-sm overflow-hidden rounded-2xl border border-white/10 bg-[#061020] p-5 shadow-2xl shadow-black/40">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div>
            <h2 className="text-base font-black text-slate-100">Sincronizar NF-es Recebidas</h2>
          ...
          <div className="space-y-3">
            ...
            {selectedClienteId && (
              <>
                ...
                <label className="grid gap-1.5">
                  <span className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-500">Ambiente</span>
                  <select
                    className="min-h-10 rounded-lg border border-white/10 bg-slate-950/60 px-3 text-xs text-slate-100 outline-none disabled:opacity-60"
                    value={environment}
                    onChange={(e) => setEnvironment(e.target.value as "producao" | "homologacao")}
                    disabled={isSyncing}
                  >
